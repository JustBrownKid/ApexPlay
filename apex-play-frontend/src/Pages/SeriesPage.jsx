...
import { Search, Tv, X, SlidersHorizontal } from 'lucide-react';
...
const SeriesPage = () => {
    ...
    const filteredSeries = seriesList.filter(item => {
        ...
        const matchesCategory = selectedCategory === 'All' ||
            item.categories?.some(c => c.category.name === selectedCategory);
        return matchesSearch && matchesCategory;
    });

    if (loading) return <Loading />;
    if (error) return <ErrorPage message={error} onRetry={fetchSeries} />;
            ...
            <main className="max-w-7xl mx-auto">
                {filteredSeries.length > 0 ? (
                    ...
                    <div className="text-center py-32 border-2 border-dashed border-white/5 rounded-3xl">
                        ...
                        <h3 className="text-slate-500 font-black uppercase tracking-widest">No Series Found</h3>
                        <p className="text-slate-700 text-xs font-bold mt-2 uppercase">Try adjusting your filters</p>
                    </div>
                )}
            </main>
